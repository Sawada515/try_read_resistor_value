...
def process_all_images_to_csv(output_csv_path: str = "dataset_features.csv"):
    """
    ディレクトリ内の全画像を処理し、特徴量をリストに蓄積してから最後にCSV出力する
    """
    input_path = Path(IMG_INPUT_DIR)
    files = list(input_path.rglob("*.bmp"))
    ...
    # 全データを保持するリスト
    all_features_data: List[Dict[str, Any]] = []
    ...
    for file in files:
        ...
        # --- 特徴量計算 (EstimateBandのリストを取得) ---
        # 引数は前回の修正通り 2Dセグメントのみ
        features_list = calculate_band_features(result_bands, lab_segment)

        # --- 辞書化 & リストへの蓄積 ---
        for i, (band, feat) in enumerate(zip(result_bands, features_list)):
            
            # 1. dataclass を辞書に変換
            row_dict = asdict(feat)  # {'band_rate': ..., 'symmetry': ..., ...}
            
            # 2. ファイル名や座標情報(DetectBand由来)を追加
            row_dict["filename"] = f"{file_count:03d}.bmp"
            row_dict["band_id"] = i
            row_dict["x_center"] = band.x
            row_dict["width"] = band.band_width
            row_dict["energy"] = band.energy_score
            
            # 3. 教師ラベル用カラム (空欄)
            row_dict["label"] = ""

            # 4. 全データリストに追加
            all_features_data.append(row_dict)
        
        file_count = file_count + 1
        
        print(f"  > {file.name}: {len(result_bands)} bands stored.")

    # --- ループ終了後: CSV書き出し ---
    if all_features_data:
        # 辞書のキーをヘッダーとして利用 (順序を整える)
        fieldnames = [
            "filename", "band_id", "x_center", "width",
        ...
        print(f"\nSaved {len(all_features_data)} rows to {output_csv_path}")
    else:
        print("No data found.")

if __name__ == "__main__":
    process_all_images_to_csv()
