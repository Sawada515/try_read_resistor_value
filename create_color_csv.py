...
def main():
    classifier = joblib.load("band_binary_classifier.pkl")
...
def create_filtered_color_dataset(output_csv_path: str = "dataset_color_features.csv"):
    ...
    try:
        ...
        print("Error: band_binary_classifier.pkl が見つかりません。先に学習を行ってください。")
        return
    ...
    for file in files:
        ...
        if not check_resistor_roi_quality(img): continue
        clipped_roi = clip_resistor_roi(img)
        if clipped_roi is None: continue
        
        h, w = clipped_roi.shape[:2]
        specular_mask = create_specular_mask(clipped_roi)
        if specular_mask is None: continue
        
        lab_segment, valid_ratio = select_best_segment(clipped_roi, specular_mask)
        if lab_segment is None: continue
        
        lab_one_line = compress_segment_to_one_line(lab_segment)
        bg_stats = analyze_resistor_background(lab_one_line, valid_ratio)
        
        result_bands, _ = detect_bands(lab_one_line, bg_stats)
        ...
        # 1. 物理特徴量抽出 (AI判定用)
        phys_features = calculate_band_features(result_bands, lab_segment)
        
        valid_bands = []
        
        # 2. 推論して選別
        for band, feat in zip(result_bands, phys_features):
            # [band_rate, symmetry, log_norm, std_l, energy]
            feature_vector = [[
                feat.band_rate,
                feat.symmetry,
        ...
        for band, feat in zip(valid_bands, color_features):
            row = {
                "filename": f"{file_count:03d}.bmp",
                "band_id":  valid_bands.index(band), # インデックス再計算
                "x_center": band.x, # アノテーション用にdetect_bandから座標を取得
                # 特徴量 (BandColorの中身を展開)
                "mean_l": f"{feat.mean_l:.2f}",
                "mean_a": f"{feat.mean_a:.2f}",
                "mean_b": f"{feat.mean_b:.2f}",
                "mean_c": f"{feat.mean_c:.2f}",
                "std_l":  f"{feat.std_l:.2f}",
                # ラベル記入欄
                "label_name": "", 
                "label_id": ""
            }
        ...
        print(f"  > {file.name}: {len(valid_bands)} bands extracted (AI Filtered).")

    # --- CSV書き出し ---
    if data_rows:
        header = [
            "filename", "band_id", "x_center",
            "mean_l", "mean_a", "mean_b", "mean_c", "std_l",
            "label_name", "label_id"
        ]
        ...
        print(f"\nSaved filtered color dataset to: {output_csv_path}")
        print(f"Total rows: {len(data_rows)}")
    else:
        print("No bands found after AI filtering.")

if __name__ == "__main__":
    create_filtered_color_dataset()